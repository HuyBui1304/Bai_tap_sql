...
def them():
    ...
    # Kiểm tra xem có ID nào đã bị xóa hay không
    c.execute('''
        SELECT id + 1 FROM student s 
        WHERE NOT EXISTS (
            SELECT 1 FROM student s2 WHERE s2.id = s.id + 1
    ...
    # Nếu tìm thấy khoảng trống trong ID, chèn sinh viên với ID đó
    if result:
        next_id = result[0]
        c.execute('''
        ...
        # Nếu không có khoảng trống, chèn bình thường để SQLite tự tăng ID
        c.execute('''
